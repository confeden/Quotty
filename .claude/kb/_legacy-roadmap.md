# SUPERSEDED - verbatim pre-restructure ROADMAP
...
Repo: `https://github.com/confeden/Quotty` (public, GPL-3.0, default branch `main`). **v1.0.0 is published**; its installer asset was later replaced in place with the Antigravity-IDE-fix build (0 downloads at the time). The owner edits README.md directly on GitHub — always fetch/rebase before pushing.
...
- **Antigravity IDE ships a differently-named server and announces itself nowhere.** The 2.0 app runs `language_server.exe` and logs its port and `--csrf_token`; the IDE runs `language_server_windows_x64.exe` and writes neither the Electron log nor a daemon descriptor (`~/.gemini/antigravity-ide/daemon/ls_*.json` on this machine is from March). The only source is the processes themselves: port from `GetExtendedTcpTable`, token from `NtQueryInformationProcess(ProcessCommandLineInformation)`. Both work from a normal-user process against an **elevated** IDE — `PROCESS_QUERY_LIMITED_INFORMATION` is granted up the integrity ladder, unlike `PROCESS_VM_READ`, so never read the PEB for this.
- **The server's HTTPS port is the lower of an adjacent pair**; its third port speaks LSP and would swallow a TLS handshake until the timeout, so only the pair member is tried.
...
- **This machine shows different filesystem contents to elevated and normal-user processes at the same path.** Proven with a marker file: `%APPDATA%\Quotty\probe_elevated.txt` written from the elevated session is invisible to an Explorer-launched process, and `%APPDATA%\Claude` has the full Desktop data in the elevated view but only `CLAUDE.md` in the normal one. Consequences: (a) editing settings.json from the agent's shell does **not** reach the app when the app runs normally — write it through an Explorer-launched helper; (b) a normally-launched Quotty reports "Claude data dir not found" here while Codex and Antigravity work; (c) test UI behaviour with an Explorer-launched instance, not one started from the agent's shell.
- **Settings must survive a UTF-8 BOM** — PowerShell's `Set-Content -Encoding UTF8` writes one and `serde_json` then fails at "line 1 column 1". `load()` trims it and logs why it fell back to defaults.
- **Only persist the window position after a drag we started.** Saving on any pointer release let a stray click bake in a position the user never chose.
...
## Open issues / Plans
- Release v1.0.0 is still a draft; `releases/latest` 404s until it is published, which the update check treats as "up to date".
- `ROADMAP.md`, `RELEASE-1.0.0.md` and the ignore rules are deliberately **not** in the repo (owner): ignores live in `.git/info/exclude`, so there is no tracked `.gitignore`.
...
- Seen once: a right-click on the strip a few seconds after launch did not open settings; the next click did. Not reproduced by hand — watch for it.
- Claude quota is unavailable to a normally-launched Quotty on this machine (see the filesystem-view gotcha). Unresolved: what hides `%APPDATA%\Claude` from normal-user processes — the ACL grants the user full control, and the directory is not a reparse point.
