...
fn from_main_log(path: &Path) -> Option<Endpoint> {
    ...
    let csrf = {
        let at = raw.rfind("--csrf_token=")?;
        raw[at + "--csrf_token=".len()..]
            .chars()
            .take_while(|c| c.is_ascii_alphanumeric() || *c == '-')
            .collect::<String>()
    };
    let port = {
        let at = raw.rfind("127.0.0.1:")?;
        raw[at + "127.0.0.1:".len()..]
            .chars()
...
fn call_quota_summary(ep: &Endpoint) -> Result<QuotaSummaryData, String> {
    ...
    let parsed: QuotaSummaryResponse =
        resp.into_json().map_err(|e| format!("parse quota summary: {e}"))?;
...
mod tests {
    ...
    fn parse_quota_summary_with_weekly_and_5h() {
        ...
        assert_eq!(w.resets_at - w.start.unwrap(), chrono::Duration::seconds(5 * 3600));
